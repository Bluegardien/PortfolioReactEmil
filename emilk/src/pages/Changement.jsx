...
function Galleries() {
    ...
    useEffect(() => {
        const fetchImages = async () => {
            try {
                const resCafe = await fetch("http://localhost:3001/api/view/cafe");
                const resMatcha = await fetch("http://localhost:3001/api/view/matcha");
                const resLatte = await fetch("http://localhost:3001/api/view/latte");
    ...
    const handleDelete = async (type, filename) => {
        try {
            const res = await fetch(`http://localhost:3001/api/${type}/delete/${filename}`, { method: 'DELETE' });
    ...
    const handleUpload = async (e) => {
        ...
        try {
            const res = await fetch(`http://localhost:3001/api/${uploadType}/upload`, {
                method: 'POST',
                body: formData
            });
            const result = await res.json();
